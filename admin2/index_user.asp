...
<%
if isnul(rCookie("adminName")) then response.Redirect("login.asp"):response.End()
...
if not isnul(id) then
	...
	if not rsObj.eof then 
		...
		wCookie"languagePath",rsObj("LanguagePath")
		wCookie"LanguageAlias",rsObj("Alias")
	else		
...
if not isnul(SceneID) then
	...
	if not rsObj.eof then 
		'wCookie"SceneID",id
		wCookie"SceneMenu",rsObj("SceneMenu")
	else		
...
<title><%=setting.siteTitle%>-网站内容管理系统- Powered by AspCms2</title>
</head>
...
<DIV class=menu style="FLOAT: right" onClick="window.open('../')">网站首页</DIV>
      ...
      <%
		...
		do while not rs.eof 
			if cint(rCookie("languageID"))=cint(rs(0)) then 
