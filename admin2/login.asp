...
'IsAdmin
	...
	if rsObj(0)=1 then
		...
		Set rsObj=Conn.Exec("select IsAdmin, GroupStatus,GroupName, UserStatus, UserID, GroupMenu, LanguageID,adminrand from {prefix}User as a, {prefix}UserGroup as b where LoginName='"&UserName&"' and a.GroupID=b.GroupID","r1")	
		if not rsObj.Eof Then
			...
			wCookie"adminrand",rsObj("adminrand")	
			wCookie"languageID",LanguageID			
			wCookie"LanguagePath",rs("LanguagePath")
			wCookie"LanguageAlias",rs("Alias")
...
<title><%=setting.siteTitle%>-开源企业网站管理系统</title>
<style type="text/css">
<!--
* {
	padding:0px;
  ...
  <div class="login">
    <form method="post" action="?action=login" onSubmit="writeCookie('username',this.username.value, 1)">
      ...
      <div class="inputbox">
        ...
        <dl>
          ...
          <dd>
            ...
            <img src="../inc/checkcode.asp" id="SeedImg" align="absmiddle" style="cursor:pointer;" border="0" alt="点我刷新" title="点我刷新" onClick="changeimg()" /> </dd>
        </dl>
        <%end if%>
        <dl>
          <dt>类型：</dt>
          <dd>
            <input name="atype" type="radio" value="1" disabled="disabled" />
            简单版
            <input name="atype" type="radio" value="0" checked="checked" />
            全功能版 </dd>
        </dl>
        <dl>
          <dt>&nbsp;</dt>
      ...
      <div class="butbox">
        <dl>
          <dt></dt>
          <dd> <span style="color:#505050;font-size:12px;display:block"><a href="http://www.red-idea.net" target="_blank" style="color:#315D80">开源创新实验室</a> 推出的以企业建站为基础的专业建站系统，帮助其实现一站式服务。</span> <a href="/">返回首页</a></dd>
        </dl>
      </div>
    </form>
...
<div class="copyright"> Powered by <a href="#">red-idea.net</a> Copyright &copy;2010-2011 </div>
</body>
</html>
