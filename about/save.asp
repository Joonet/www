...
dim FaqID,FaqTitle,Contact,ContactWay,Content,Reply,AddTime,ReplyTime,FaqStatus,AuditStatus,tab,sql,companyname
Sub addGbook
	...
	FaqTitle= filterPara(getForm("companyname","post"))
	Contact=filterPara(getForm("Contact","post"))
	ContactWay=filterPara(getForm("ContactWay","post"))
	'companyname=filterPara(getForm("companyname","post"))
	...
	if isnul(Contact) then alertMsgAndGo "No Name!","-1"
	if isnul(Content) then alertMsgAndGo "No Content!","-1"
	if isnul(ContactWay) then alertMsgAndGo "No Email!","-1"
