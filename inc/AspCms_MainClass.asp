...
Class TemplateClass
	...
	Public Function parseGlobal()		
		...
		if rCookie("GroupID")="1" then
			content=replaceStr(content,"{aspcms:userright}",0)
		end if
		if rCookie("GroupID")="2"or rCookie("GroupID")="" then
			content=replaceStr(content,"{aspcms:userright}",2)
		end if
		if rCookie("GroupID")="3" then
					...
					if str="content" or str="aboutcontent" or str="news" or str="product" or str="down" or str="pic"  then							
						...
						select case fieldName
							...
							case "indexvideo"
								nloopstr = replace(nloopstr,matchfield.value,rsObj("ContentSource"))
							case "videourl"
		if rCookie("GroupID") = "" then
			wCookie"GroupID",2
		end if
			if rCookie("GroupID") > 1 then
				if rsObj("VideoGroupID")>rCookie("GroupID") then
				nloopstr = replace(nloopstr,matchfield.value,"待定视频.avi")
				else
				nloopstr = replace(nloopstr,matchfield.value,rsObj("ContentSource"))
				end if
			else
	...
	Function getqqkf2
		...
		if not isnul(serviceQQ) then
			...
			for i=0 to ubound(tempstr)				
				if isExistStr(tempstr(i),"|") then	
					Str=Str&" <div class=kefu_box><span  style=""padding-left:10px;"">"&split(tempstr(i),"|")(0)&"</span></div><DIV class=kefu_box onmouseover=""this.className='kefu_boxOver'"" onmouseout=""this.className='kefu_box'""><A class=kefu_Type_qq href=""tencent://message/?uin="&trim(split(tempstr(i),"|")(1))&"&amp;Menu=yes""><img border=""0"" src=""http://wpa.qq.com/pa?p=2:"&trim(split(tempstr(i),"|")(1))&":41 &r=0.8817731731823399"" alt=""send message"" title=""send message""></A></DIV>"&vbcrlf
				else
					Str=Str&" <div class=kefu_box><span  style=""padding-left:10px;"">salesman:</span></div><DIV class=kefu_box onmouseover=""this.className='kefu_boxOver'"" onmouseout=""this.className='kefu_box'""><A class=kefu_Type_qq href=""tencent://message/?uin="&trim(tempstr(i))&"&amp;Menu=yes""><img border=""0"" src=""http://wpa.qq.com/pa?p=2:"&trim(tempstr(i))&":41 &r=0.8817731731823399"" alt=""send message"" title=""send message""></A></DIV>"&vbcrlf				
				end if
			next
		end if
	...
	Public Function parsePrevAndNext(Id,SortID)
		...
		if rsObjNext.eof then 
			...
			content = replace(content,"{aspcms:nexttitle}","no message!")
		else			
