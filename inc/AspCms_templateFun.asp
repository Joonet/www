...
function makeContent(Byval contentID, Byval page, Byval isMakeHtml)
	...
	templateObj.content=replace(templateObj.content,"["&str&":source]",repnull(rsObj("ContentSource")))
		if rCookie("GroupID") = "" then
			...
			if rCookie("GroupID") > 1 then
				...
				templateObj.content=replace(templateObj.content,"["&str&":videourl]","待定视频.avi")
				else
	...
	if rCookie("GroupID") = "" then
		...
		if rCookie("GroupID") > 1 then
			if rsObj("DownGroupID")>rCookie("GroupID") then
			templateObj.content=replace(templateObj.content,"["&str&":downurl]","<a href=""#"" onclick=""javascript:alert('Forbid to download')"">Download</a>")
			else
			templateObj.content=replace(templateObj.content,"["&str&":downurl]","<a href="&repnull(rsObj("DownURL"))&" >Download</a>")
			end if
		else
		templateObj.content=replace(templateObj.content,"["&str&":downurl]","<a href="&repnull(rsObj("DownURL"))&" >Download</a>")
		end if	
