...
sas.ued.FlashObject = function (swf, id, w, h, ver, c, useExpressInstall, quality, xiRedirectUrl, redirectUrl, 
detectKey)
{
    ...
    if (ver)
    {
        this.setAttribute('version', new sas.ued.PlayerVersion(ver.toString().split(".")));
    }
...
sas.ued.FlashObjectUtil.getPlayerVersion = function (reqVer, xiInstall)
{
    ...
    if (navigator.plugins && navigator.mimeTypes.length)
    {
        ...
        if (x && x.description)
        {
            PlayerVersion = new sas.ued.PlayerVersion(x.description.replace(/([a-z]|[A-Z]|\s)+/, "").replace(/(\s+r|\s+b[0-9]+)/, 
            ".").split("."));
        }
    }
    else
